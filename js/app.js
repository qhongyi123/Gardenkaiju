...
function initDevMode() {
    var savedDevMode = localStorage.getItem('garden-dev-mode');
    devMode = (savedDevMode === '1');
    var cb = document.getElementById('setting-dev-mode');
    if (cb) cb.checked = devMode;
    var devTabBtn = document.getElementById('dev-tab-btn');
    if (devTabBtn) devTabBtn.style.display = devMode ? '' : 'none';
    var devTabBtn2 = document.getElementById('dev-tab-btn2');
    if (devTabBtn2) devTabBtn2.style.display = devMode ? '' : 'none';
    var savedReqs = localStorage.getItem('garden-dev-requirements');
    if (savedReqs) {
        try { devRequirements = JSON.parse(savedReqs); } catch(e) { devRequirements = DEFAULT_REQUIREMENTS.slice(); }
    } else {
        devRequirements = DEFAULT_REQUIREMENTS.slice();
    }
    renderRequirements();
}
...
function toggleDevMode(checked) {
    devMode = checked;
    localStorage.setItem('garden-dev-mode', checked ? '1' : '0');
    var devTabBtn = document.getElementById('dev-tab-btn');
    if (devTabBtn) devTabBtn.style.display = checked ? '' : 'none';
    var devTabBtn2 = document.getElementById('dev-tab-btn2');
    if (devTabBtn2) devTabBtn2.style.display = checked ? '' : 'none';
}

function renderRequirements() {
    var container = document.getElementById('dev-requirements');
    if (!container) return;
    var html = '';
    devRequirements.forEach(function(req, i) {
        html += '<div class="dev-req-item">';
        html += '<span class="dev-req-num">' + (i + 1) + '.</span>';
        html += '<input type="text" class="dev-req-input" value="' + escapeHtml(req) + '" oninput="updateRequirement(' + i + ', this.value)">';
        html += '<button class="dev-req-delete" onclick="deleteRequirement(' + i + ')" title="删除">✕</button>';
        html += '</div>';
    });
    container.innerHTML = html;
}
...
function addRequirement() {
    devRequirements.push('');
    saveRequirements();
    renderRequirements();
}

function updateRequirement(index, value) {
    devRequirements[index] = value;
    saveRequirements();
}

function deleteRequirement(index) {
    devRequirements.splice(index, 1);
    saveRequirements();
    renderRequirements();
}

function saveRequirements() {
    localStorage.setItem('garden-dev-requirements', JSON.stringify(devRequirements));
}

function sendOpeningGeneration() {
    var lines = [];
    lines.push('生成开场白');
    var reqs = devRequirements.filter(function(r) { return r.trim() !== ''; });
    if (reqs.length > 0) {
        lines.push('要求：');
    ...
    var plot = document.getElementById('dev-plot');
    if (plot && plot.value.trim()) {
        lines.push('剧情：');
        lines.push(plot.value.trim());
    }
...
function parseQyText() {
    var input = document.getElementById('proc-raw-input');
    var preview = document.getElementById('proc-preview-content');
    if (!input || !preview) return;

    var raw = input.value.trim();
    if (!raw) {
        preview.innerHTML = '<span class="proc-preview-hint">请先粘贴文本</span>';
        return;
    }
...
function exportStartJson() {
    var idEl = document.getElementById('proc-id');
    var nameEl = document.getElementById('proc-name');
    var colorEl = document.getElementById('proc-color');
    var tagsEl = document.getElementById('proc-tags');
    var authorEl = document.getElementById('proc-author');
    var timeEl = document.getElementById('proc-time');
    var imageEl = document.getElementById('proc-image');
    var charsEl = document.getElementById('proc-related-chars');
    var modsEl = document.getElementById('proc-related-mods');

    var id = parseInt(idEl.value) || 0;
    var name = (nameEl.value || '').trim();
    var color = colorEl.value || 'soft-green';
    var tagsRaw = (tagsEl.value || '').trim();
    var author = (authorEl.value || '').trim();
    var time = (timeEl.value || '').trim();
    var image = (imageEl.value || '').trim();
    var charsRaw = (charsEl.value || '').trim();
    var modsRaw = (modsEl.value || '').trim();

    if (!id) { alert('请填写编号'); return; }
    if (!name) { alert('请填写故事名称'); return; }
    if (!parsedContent) { alert('请先解析文本'); return; }

    var tags = tagsRaw ? tagsRaw.split(',').map(function(t) { return t.trim(); }).filter(Boolean) : [];
    var relatedChars = charsRaw ? charsRaw.split(',').map(function(c) { return c.trim(); }).filter(Boolean) : [];
    var relatedMods = modsRaw ? modsRaw.split(',').map(function(m) { return m.trim(); }).filter(Boolean) : [];

    var content = parsedContent;
    var contentRaw = parsedContentRaw;

    var obj = { id: id, color: color };
...
function resetProcPanel() {
    var rawInput = document.getElementById('proc-raw-input');
    if (rawInput) rawInput.value = '';
    document.getElementById('proc-id').value = '';
    document.getElementById('proc-name').value = '';
    document.getElementById('proc-color').value = 'soft-green';
    document.getElementById('proc-tags').value = '';
    document.getElementById('proc-author').value = '';
    document.getElementById('proc-time').value = '';
    ...
    parsedContent = '';
    var preview = document.getElementById('proc-preview-content');
    if (preview) preview.innerHTML = '<span class="proc-preview-hint">点击「解析文本」后在此预览</span>';
    setProcType('start');
}
