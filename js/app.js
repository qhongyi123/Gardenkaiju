...
function renderStartCards(containerId, dataArray) {
    ...
    dataArray.forEach(function(card) {
        ...
        if (card.author || card.time) {
            html += '<div class="card-meta">';
            if (card.author) {
                html += '<span class="card-author">✎ ' + escapeHtml(card.author) + '</span>';
            }
            if (card.time) {
                html += '<span class="card-time">' + escapeHtml(card.time) + '</span>';
            }
            html += '</div>';
        }
...
function openModal(cardData) {
    ...
    var displayContent = cardData.content || '暂无内容';
    var previewText = displayContent.length > 80 ? displayContent.substring(0, 80) + '...' : displayContent;
...
function openViewStartModal() {
    ...
    document.getElementById('view-modal-title').textContent = currentCardData.title;
...
function sendStartContent() {
    ...
    var title = currentCardData.title || '未知开局';
    var sendText = '【' + title + '】\n\n' + rawContent;
